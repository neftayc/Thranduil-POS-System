...
-- invoice_no = IMPORT-HOJA1-EMPRESA-AYRAMPO-001
--
-- Objetivo:
-- 1) Recuperar unidad desde stg_compras_ayrampo_import
...
values
  ('ciento', 'Ciento', true),
  ('medio_ciento', '1/2 Ciento', true),
  ('cuarto_ciento', '1/4 Ciento', true),
  ('sobre', 'Sobre', true)
...
create temporary table tmp_ayrampo_purchase_before on commit drop as
select
  pi.id,
  pi.purchase_id,
  pi.product_id,
...
create temporary table tmp_ayrampo_stage_base on commit drop as
select
...
create temporary table tmp_ayrampo_purchase_base on commit drop as
select
...
create temporary table tmp_ayrampo_match_phase1 on commit drop as
with s as (
  select
    sb.*,
    row_number() over (partition by sb.sku_key, sb.qty, sb.total_cost order by sb.stage_row_id) as rn
  from tmp_ayrampo_stage_base sb
  where sb.sku_key <> ''
),
p as (
  select
  ...
  from tmp_ayrampo_purchase_base pb
  where pb.sku_key <> ''
)
select
  p.purchase_item_id,
...
create temporary table tmp_ayrampo_match_phase2 on commit drop as
with s0 as (
  select *
  from tmp_ayrampo_stage_base
  where stage_row_id not in (select stage_row_id from tmp_ayrampo_match_phase1)
),
p0 as (
  select *
  from tmp_ayrampo_purchase_base
  where purchase_item_id not in (select purchase_item_id from tmp_ayrampo_match_phase1)
),
s as (
  select
    s0.*,
...
create temporary table tmp_ayrampo_match_phase3 on commit drop as
with s0 as (
  select *
  from tmp_ayrampo_stage_base
  where stage_row_id not in (
    select stage_row_id from tmp_ayrampo_match_phase1
    union
    select stage_row_id from tmp_ayrampo_match_phase2
  )
),
p0 as (
  select *
  ...
  where purchase_item_id not in (
    select purchase_item_id from tmp_ayrampo_match_phase1
    union
    select purchase_item_id from tmp_ayrampo_match_phase2
  )
),
s as (
  select
...
create temporary table tmp_ayrampo_matched on commit drop as
select purchase_item_id, stage_row_id, unit_raw from tmp_ayrampo_match_phase1
union all
select purchase_item_id, stage_row_id, unit_raw from tmp_ayrampo_match_phase2
union all
select purchase_item_id, stage_row_id, unit_raw from tmp_ayrampo_match_phase3;

with matched as (
  select
    purchase_item_id,
    unit_raw
  from tmp_ayrampo_matched
)
update public.purchase_items pi
set
  unit_name = case
    when m.unit_raw in ('caja', 'cajas', 'cajita', 'cajitas') then 'caja'
    when m.unit_raw in ('1/2 ciento', 'medio ciento', 'medio_ciento') then 'medio_ciento'
    when m.unit_raw in ('1/4 ciento', 'cuarto ciento', 'cuarto_ciento') then 'cuarto_ciento'
...
declare
  ...
  v_changed int := 0;
begin
  select count(*) into v_total_items from tmp_ayrampo_purchase_before;
  select count(*) into v_matched_items from tmp_ayrampo_matched;

  if v_total_items = 0 then
    raise exception 'No se encontraron items para invoice IMPORT-HOJA1-EMPRESA-AYRAMPO-001';
  end if;
  ...
  select count(*)
    into v_changed
  from public.purchase_items pi
  join tmp_ayrampo_purchase_before b on b.id = pi.id
  where
...
-- select pb.*
-- from tmp_ayrampo_purchase_base pb
-- where pb.purchase_item_id not in (select purchase_item_id from tmp_ayrampo_matched)
